<x-layouts.admin>
            ...
            <tbody>
                ...
                @foreach ($permissions as $permission)
                    <tr class="{{ $loop->even ? 'bg-blue-100 dark:bg-gray-700' : 'bg-white dark:bg-gray-800' }} border-b border-gray-200 dark:border-gray-700">
                        <th class="px-6 py-4 font-medium whitespace-nowrap">{{ $permission->id }}</th>
                        <td class="px-6 py-4">
                            <a href="{{ route('permissions.show', $permission) }}" class="text-blue-700 dark:text-blue-300 font-semibold hover:underline">
                                {{ $permission->name }}
                            </a>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ route('permissions.edit', $permission) }}" class="text-blue-600 hover:text-blue-900 font-semibold">Edit</a>
                        </td>
                        <td class="px-6 py-4">
                            <form class="delete-form" action="{{ route('permissions.destroy', $permission) }}" method="POST">
                                @csrf
                                @method('DELETE')
    ...
    <div class="mt-6">
        <a href="{{ route('permissions.create') }}" class="inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 font-semibold shadow">
            + Add Permission
        </a>
    </div>
